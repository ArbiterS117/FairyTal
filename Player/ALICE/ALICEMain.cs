...
public class ALICEMain : MonoBehaviour {

    GameCtrl gameCtrl;
	ALICESpecialAction playerCtrl;
    ...
    void Awake() {
        ...
        playerCtrl = GetComponent<ALICESpecialAction>();
	}
	...
	void Update() {
        ...
        if (inputEnabled) {
            ...
            if (inputDelay)
            {
                ...
                //招式
                if (Input.GetButtonDown(playerULTString) && !playerCtrl.DownState && !playerCtrl.UpState)
                {
                    if (playerCtrl.grounded)
                    {
                        if (!ULT1Ready) playerCtrl.actionSPAttack1();
                        else if (!playerCtrl.isInCombo) playerCtrl.actionULT1();

                        inputDelay = false;
                    }
                    else
                    {
                        if (!ULT1Ready) playerCtrl.actionSPAttack4();
                        else if (!playerCtrl.isInCombo) playerCtrl.actionULT1();
                ...
                if (Input.GetButtonDown(playerULTString) && playerCtrl.UpState)
                {
                    ...
                    {
                        if (!ULT1Ready) playerCtrl.actionSPAttackAquaJump();
                        else if (!playerCtrl.isInCombo) playerCtrl.actionULT1();
                ...
                {
                    if (!ULT1Ready || playerCtrl.isInCombo) playerCtrl.actionSPAttack5(true);
                    else playerCtrl.actionULT1();
                }
                else if (!Input.GetButton(playerULTString))
                {
                    playerCtrl.isLimitBreak = false;
                }
