...
public class ALADDINAIPlayerCtrl : AIPlayerBase
{
    ALADDINSpecialAction playerSpecial;

    public override void Awake()
    {
        base.Awake();
        playerSpecial = GetComponent<ALADDINSpecialAction>();
    }
                  ...
                  && this.transform.position.x - TargetTransform.position.x <= followDistance)
            {
                ...
                if (attackContinueCTime >= attackContinueColdTime)
                {
                    ...
                    else if (randomNUM <= 4.5f)
                    {
                        ...
                        else  playerSpecial.actionSPAttack6();
                    }
                    ...
                    {
                        playerSpecial.actionSPAttack6();
                    }
                    else if (randomNUM <= 7.5f)
                    {
                        playerSpecial.actionSPAttack11();
                    }
    ...
    public override void BacktoMapMid()
    {
        ...
        if (BackAirJumpColdCTime >= BackAirJumpColdTime)
        {
            ...
            else playerSpecial.actionSPAttack11();
            BackAirJumpColdCTime = 0.0f;
        }
    }
    #endregion
    ...
    public override void FollowPokeAttatk()
    {
        ...
        if (FollowAttackColdCTime >= FollowAttackColdTime)
        {
            ...
            if (randomNUM < 1.0f)
            {
                if (playerCtrl.MP >= playerSpecial.MagicCarpetJumpMP) playerSpecial.actionSPAttack6();
            }
        }
    }
    #endregion
