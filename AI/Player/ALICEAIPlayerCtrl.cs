...
public class ALICEAIPlayerCtrl : AIPlayerBase {


    ALICESpecialAction playerSpecial;

    public float LimitBreakDuration = 3.0f;
    protected float LimitBreakCDuration = 0.0f;

    public override void Awake()
    {
        base.Awake();
        playerSpecial = GetComponent<ALICESpecialAction>();
    }
        ...
        if (TargetTransform != null)
        {
                  ...
                  && this.transform.position.x - TargetTransform.position.x <= followDistance)
            {
                ...
                if (attackContinueCTime >= attackContinueColdTime)
                {
                    ...
                    {
                        if(playerCtrl.grounded)playerSpecial.actionSPAttack1();
                        else playerSpecial.actionSPAttack4();
                    }
    ...
    public override void BacktoMapMid()
    {
        ...
        if (BackAirJumpColdCTime >= BackAirJumpColdTime)
        {
            ...
            else playerSpecial.actionSPAttackAquaJump();
            BackAirJumpColdCTime = 0.0f;
        }
    }
    #endregion
    ...
    public override void FollowPokeAttatk()
    {
        ...
        if (FollowAttackColdCTime >= FollowAttackColdTime)
        {
            ...
            if (randomNUM < 1.0f)
            {
                if (playerCtrl.MP >= playerSpecial.WindWalkerMP)
                {
                    if(playerCtrl.grounded) playerSpecial.actionSPAttack1();
                    else playerSpecial.actionSPAttack4();
                }
                else playerSpecial.actionSPAttack5(true); // 開始LB
            }
        }

        //==========LimitBreak==============
        if (playerSpecial.isLimitBreak)
        {
            LimitBreakCDuration += _deltaTime;
            if(LimitBreakCDuration >= LimitBreakDuration)
            {
                LimitBreakCDuration = 0.0f;
                playerSpecial.isLimitBreak = false;
            }
        }
