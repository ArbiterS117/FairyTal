...
public class ColliderKnockOut : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D other) {
		if (other.CompareTag("PlayerDMG")) {
			...
			if(playerCtrl.tag != enemyCtrl.tag && playerCtrl.isFront == enemyCtrl.isFront && !playerCtrl.hittedPlayer[enemyCtrl.PlayerNUM - 1] && playerCtrl.teamNum != enemyCtrl.teamNum)
            {
				...
				if(!playerCtrl.getATKData().twoSide)enemyCtrl.actionKnockOuted(playerCtrl.getATKData().sideType , playerCtrl.getATKData().ATK, playerCtrl.getATKData().knockOutTime, playerCtrl.dir, playerCtrl.getATKData().knockBackSpeedX, playerCtrl.getATKData().hitForceY, playerCtrl.PlayerNUM , playerCtrl.getATKData().knockOutGravity, playerCtrl.getATKData().knockOutDecressSpeed);
				else {
					if(playerCtrl.transform.position.x >= enemyCtrl.transform.position.x)enemyCtrl.actionKnockOuted(playerCtrl.getATKData().sideType , playerCtrl.getATKData().ATK, playerCtrl.getATKData().knockOutTime, -1.0f, playerCtrl.getATKData().knockBackSpeedX, playerCtrl.getATKData().hitForceY, playerCtrl.PlayerNUM, playerCtrl.getATKData().knockOutGravity, playerCtrl.getATKData().knockOutDecressSpeed);
					else enemyCtrl.actionKnockOuted(playerCtrl.getATKData().sideType , playerCtrl.getATKData().ATK, playerCtrl.getATKData().knockOutTime, 1.0f, playerCtrl.getATKData().knockBackSpeedX, playerCtrl.getATKData().hitForceY, playerCtrl.PlayerNUM, playerCtrl.getATKData().knockOutGravity, playerCtrl.getATKData().knockOutDecressSpeed);
				}
