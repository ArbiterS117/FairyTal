...
public class ALADDINMain : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        playerCtrl = GetComponent<ALADDINSpecialAction>();
    }
    ...
    void Update()
    {
        ...
        if (inputEnabled)
        {
            ...
            if (inputDelay)
            {
                ...
                else if (Input.GetButtonUp(playerULTString))
                {
                    playerCtrl.isStorgedPower = false;
                    if (playerCtrl.canStorgedPower) playerCtrl.closeStorgedPower = true; // 接技時 手指提早放開 用另一個變數從內部關閉isStorgedPower

                    inputDelay = false;
                }

                if (Input.GetButton(playerULTString)) playerCtrl.ULTBtnPressed = true;
                else playerCtrl.ULTBtnPressed = false;

                if (Input.GetButtonDown(playerULTString) && playerCtrl.UpState)
                {
                    if (!ULT1Ready) playerCtrl.actionSPAttack11();
                    else playerCtrl.actionULT1();
                ...
                {
                    if (!ULT1Ready) playerCtrl.actionSPAttack6();
                    else playerCtrl.actionULT1();
    ...
    {
        if ((move > 0 && PrevMoveState <= 0) || (move < 0 && PrevMoveState >= 0))
        { //雙按移動衝刺判斷
            DashReadyMid = true;
        }

        if ((DashReadyMid && move == 0 && PrevMoveState > 0))
        { //雙按移動衝刺判斷
            DashReadyR = true;
            DashReadyMid = false;
        }
